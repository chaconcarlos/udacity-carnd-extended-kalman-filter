...
{
  fusionEKF.ProcessMeasurement(measurementPackage);

   VectorXd estimate(STATE_VECTOR_SIZE);

   double p_x = fusionEKF.getKalmanFilter().x_(PX_VECTOR_INDEX);
   double p_y = fusionEKF.getKalmanFilter().x_(PY_VECTOR_INDEX);
   double v1  = fusionEKF.getKalmanFilter().x_(VX_VECTOR_INDEX);
   double v2  = fusionEKF.getKalmanFilter().x_(VY_VECTOR_INDEX);
